...
function Auth() {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    try{
      const req = await axios.post('http://localhost:3333/api/login', {
        email,
        password
      });
    ...
    if (err.response?.data?.message) {
      setMsg(err.response.data.message); // example: "email already exists"
    } else if (err.response?.data?.errors?.[0]?.message) {
      setMsg(err.response.data.errors[0].message); // Joi validation
    } else {
  ...
  return (
 <div className='bg-blue-400 h-[100vh] w-full fixed'>
  <h1 className='flex absolute font-bold text-3xl text-white ml-[42%] mt-3'>Welcome Back</h1>
    <StyledWrapper>
      <form 
      onSubmit={handleLogin}
      className="form m-auto mt-15 shadow-2xl">
        ...
        <button className="button-submit bg-blue-400">Sign In</button>
